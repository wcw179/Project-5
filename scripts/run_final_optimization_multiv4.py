    ...
    try:
        ...
        for fold, (train_idx, val_idx) in enumerate(cv_splits):
            try:
                ...
                # Drop symbol column for training
                X_train_numeric = X_train.drop(columns=['symbol'])
                X_val_numeric = X_val.drop(columns=['symbol'])
...
def train_final_ensemble_model(X: pd.DataFrame, y: pd.Series,
    ...
    final_model.fit(X.drop(columns=['symbol']), y_mapped)
...
def main():
    ...
    y_full_df = pd.read_parquet(y_full_path)
    y = y_full_df['label']
    ...
    # Align datasets based on their original datetime index
    ...
    # Prepare data for PurgedStratifiedKFold
    ...
    # Create PurgedStratifiedKFold instance with correct parameters
